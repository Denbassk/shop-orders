...
$BandIndex = -1        # -1 = нижний (широкий) вариант, 0 = верхний
...
# приводим к 32bpp - иначе SetPixel/Clone падают на палитровых PNG
...
$bg = $img.GetPixel(0,0)
Write-Host "Размер: $W x $H, фон: $($bg.R),$($bg.G),$($bg.B)"
...
if ($rows.Count -eq 0) { Write-Host "Ничего не найдено"; exit }
...
Write-Host "Полос найдено: $($bands.Count)"
for ($i=0; $i -lt $bands.Count; $i++) { Write-Host "  [$i] строки $($bands[$i][0])-$($bands[$i][1])" }
...
Write-Host "Обрезано до: $cw x $ch"
...
# серую подложку заливаем белым - шапка белая, кромки сольются без ореола
